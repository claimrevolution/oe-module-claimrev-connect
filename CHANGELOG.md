...
- Add a PHPUnit 11 harness with hand-written stubs for the slice of OpenEMR core the module touches, and a GitHub Actions workflow running the suite on PHP 8.2 and 8.3. Tests drive a real Guzzle client over a `MockHandler`, so URL building, auth headers, and JSON encoding are genuinely exercised rather than mocked away. 16 tests, 34 assertions.
